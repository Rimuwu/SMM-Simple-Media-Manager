#!/bin/sh
# Entrypoint для PostgreSQL с загрузкой переменных из Vault

export VAULT_ADDR=${VAULT_ADDR:-http://vault:8200}

# Читаем токен из Docker secret
if [ -f "/run/secrets/vault_token" ]; then
    export VAULT_TOKEN=$(cat /run/secrets/vault_token)
    echo "🔐 PostgreSQL: Токен загружен из Docker secret"
else
    echo "❌ PostgreSQL: Vault token не найден!"
    exit 1
fi

echo "🔐 PostgreSQL: Загрузка переменных из Vault..."

# Ожидание доступности Vault
max_attempts=30
attempt=0
while [ $attempt -lt $max_attempts ]; do
    if nc -z vault 8200 2>/dev/null; then
        echo "✓ Vault доступен"
        break
    fi
    attempt=$((attempt + 1))
    echo "⏳ Ожидание Vault... ($attempt/$max_attempts)"
    sleep 2
done

# Установка необходимых пакетов
apk add --no-cache python3 py3-pip

# Установка hvac
pip3 install hvac --break-system-packages -q

# Загрузка переменных из Vault и экспорт
eval "$(python3 << 'EOF'
import os, sys, hvac

try:
    client = hvac.Client(
        url=os.getenv("VAULT_ADDR"), 
        token=os.getenv("VAULT_TOKEN")
    )
    
    if not client.is_authenticated():
        print("echo '❌ Не удалось аутентифицироваться в Vault'", file=sys.stderr)
        sys.exit(1)
    
    response = client.secrets.kv.v2.read_secret_version(
        path="smm", 
        mount_point="secret",
        raise_on_deleted_version=False
    )
    secrets = response["data"]["data"]
    
    # Экспортируем переменные для PostgreSQL
    for key, value in secrets.items():
        if key in ["POSTGRES_DB", "POSTGRES_USER", "POSTGRES_PASSWORD"]:
            print(f"export {key}='{value}'")
    
    print("echo '✓ PostgreSQL переменные загружены из Vault'", file=sys.stderr)
    
except Exception as e:
    print(f"echo '❌ Ошибка загрузки из Vault: {e}'", file=sys.stderr)
    sys.exit(1)
EOF
)"

if [ $? -ne 0 ]; then
    echo "❌ Не удалось загрузить переменные из Vault"
    exit 1
fi

echo "🐘 Запуск PostgreSQL с переменными:"
echo "   POSTGRES_DB=$POSTGRES_DB"
echo "   POSTGRES_USER=$POSTGRES_USER"

# Запуск стандартного entrypoint PostgreSQL
exec docker-entrypoint.sh "$@"
